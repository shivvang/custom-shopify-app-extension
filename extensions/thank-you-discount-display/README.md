...
## Metafields used by this extension

This extension reads two shop-level metafields to populate the thank-you discount UI. Create or update the following metafields in your store (Settings → Custom data → Metafields) if you want to control the displayed values from the Shopify admin:

- Namespace: `thank_you`, Key: `discount_code` — the discount code shown to the customer (e.g. `THANKYOU15`).
- Namespace: `thank_you`, Key: `discount_message` — the message shown above the code (e.g. `Special Discount Coupon For Sweet Dreams :`).

If these metafields are not present the extension will fall back to the built-in defaults declared in `src/Checkout.tsx`.
